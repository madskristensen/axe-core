 ...
 */
axe.utils.sendCommandToFrame = function(node, parameters, callback) {
  'use strict';
  ...
  axe.utils.respondable(win, 'axe.ping', null, function () {
    ...
    axe.utils.respondable(win, 'axe.start', parameters, function (data) {
      if (callback) {
        clearTimeout(timeout);
        callback(data);
      }
...
* @param  {Function} callback  Function to call when results from all frames have returned
*/
axe.utils.collectResultsFromFrames = function collectResultsFromFrames(context, options, command, parameter, callback) {
  'use strict';
  ...
  q.then(function (data) {
    callback(axe.utils.mergeResults(data));
  });
};
